...
{
  char* t_pBuff=NULL;
  t_pBuff=(char*)malloc(def_msgBuff);memRecoder(t_pBuff,1,0,110);
...
int ptnapiLldpInquery(struct td_linkedlist* v_pSocketLinkList,int v_ptnapiSocketFd,int v_portNo,int v_function)
{
  char t_tempChars[def_msgBuff]={0};
  char* t_tempStr=NULL;
  t_tempStr=(char*)malloc(def_msgBuff);memRecoder(t_tempStr,1,0,343);
  sprintf(t_tempChars,def_ptnapiPort,v_portNo);
...
{
  char* t_tempStr=NULL;
  t_tempStr=(char*)malloc(def_msgBuff);memRecoder(t_tempStr,1,0,363);
